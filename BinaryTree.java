...
public class BinaryTree<T extends Comparable<T>> {  //created generic type
	...
	public boolean find(T value) {
		...
		while (current != null) {
			if (current.getData().compareTo(value) == 0) {
				count++;
				System.out.println("binary tree compared " + count + " times");
				return true;
			} else {
				if (value.compareTo(current.getData()) > 0) {
					count++;
					// larger than the value in the current node
					// go down right branch
		...
		System.out.println("Binary tree didn't find the value " + value);
		System.out.println("Processed search " + count + " times");
		return false;
	}
	...
	public void traverse() {
		...
		// iterative inOrder traversal of the BST � notice how much more complicated it
		// is
		...
		while (tree != null) { // go down the left branch,push the node on to the stack to be revisited
			tree = tree.getLC();
			while (tree != null) {
				stack.push(tree);
				// System.out.println( "\npushing " + tree.getData() + "onto the stack");
			...
			if (tree != null)
				...
				if (tree != null)
					stack.push(tree); // push the right branch node onto the stack
			}
